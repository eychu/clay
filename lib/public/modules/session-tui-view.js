...
var resizeButtonBound = false;

function ensureResizeButtonBound() {
  if (resizeButtonBound) return;
  var btn = document.getElementById("tui-resize-btn");
  if (!btn) return;
  resizeButtonBound = true;
...
function setResizeButtonVisible(visible) {
  ensureResizeButtonBound();
  var btn = document.getElementById("tui-resize-btn");
  if (btn) btn.classList.toggle("active", !!visible);
}

if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", ensureResizeButtonBound);
} else {
  ensureResizeButtonBound();
}
